...
当前已标记版本：[v7.30](CHANGELOG.md)

版本记录：[CHANGELOG.md](CHANGELOG.md)

## 概述

福大灵犀为福州大学学生提供类 ChatGPT 的对话体验。每个学生使用学号登录后，拥有独立的对话记录，并可通过 AI 助手查询成绩、课表等教务信息；校外人员也可通过第三方访客登录使用公共问答和校园生活建议。

## 功能特性

- **学生登录认证**：基于学号的登录系统，每个学生对话完全隔离
- **独立 Passkey 登录**：无需学号、邮箱、密码或第三方账号，首次创建通行密钥即可生成独立访客身份；下次用同一通行密钥回到原身份。账号设置可添加其他密钥；请保留同步或备份，全部丢失且无其他登录方式时无法恢复。不同身份的聊天记录不会自动合并。
- **支付宝内使用**：手机打开支付宝后通过官方网页完成授权并直接进入聊天，不再依赖原生桥接接口，也不要求返回 Edge 或复制链接。诊断面板及旧中转试验接口已移除。原浏览器不会同步登录；如需在浏览器使用，可选择 Passkey。支付宝自身的外链提示仍可能出现。
...
- `FZU_CHAT_OAUTH_PROVIDERS` – 可选，控制登录页可见的访客登录方式，例如生产环境设置为 `microsoft,apple,github`，同时仓库代码仍保留微信与 QQ
- `FZU_CHAT_WECHAT_CLIENT_ID` / `FZU_CHAT_WECHAT_CLIENT_SECRET`、`FZU_CHAT_QQ_CLIENT_ID` / `FZU_CHAT_QQ_CLIENT_SECRET`、`FZU_CHAT_MICROSOFT_CLIENT_ID` / `FZU_CHAT_MICROSOFT_CLIENT_SECRET`、`FZU_CHAT_APPLE_CLIENT_ID` / `FZU_CHAT_APPLE_CLIENT_SECRET`、`FZU_CHAT_GITHUB_CLIENT_ID` / `FZU_CHAT_GITHUB_CLIENT_SECRET` – 可选，启用对应访客 OAuth 登录；可通过对应 `*_REDIRECT_URI` 覆盖回调地址，默认回调为 `/api/auth/oauth/{provider}/callback`。Apple 的 `FZU_CHAT_APPLE_CLIENT_SECRET` 使用生成后的 client-secret JWT；Microsoft 可额外设置 `FZU_CHAT_MICROSOFT_TENANT`，默认 `common`。
...
export FZU_CHAT_OAUTH_PROVIDERS=microsoft,apple,github # 可选，控制可见访客登录方式
export FZU_CHAT_GITHUB_CLIENT_ID=... # 可选，访客 GitHub 登录
...
生产环境可使用 `docker-compose.prod.yml` 启动内部 Redis。设置 URL-safe 的 `REDIS_PASSWORD`，例如 `openssl rand -hex 32`，再执行 `FZU_CHAT_VERSION=v7.18 ./scripts/deploy-ghcr.sh`；如果 GHCR 镜像拉取失败，脚本会回退到本地生产镜像构建。
...
- `FZU_CHAT_OAUTH_PROVIDERS=microsoft,apple,github` – 生产环境只展示 Microsoft、Apple 和 GitHub；不设置时展示仓库支持的全部第三方登录
- `FZU_CHAT_WECHAT_CLIENT_ID` / `FZU_CHAT_WECHAT_CLIENT_SECRET`、`FZU_CHAT_QQ_CLIENT_ID` / `FZU_CHAT_QQ_CLIENT_SECRET`、`FZU_CHAT_MICROSOFT_CLIENT_ID` / `FZU_CHAT_MICROSOFT_CLIENT_SECRET`、`FZU_CHAT_APPLE_CLIENT_ID` / `FZU_CHAT_APPLE_CLIENT_SECRET`、`FZU_CHAT_GITHUB_CLIENT_ID` / `FZU_CHAT_GITHUB_CLIENT_SECRET` – 启用对应访客登录入口；未配置时前端按钮会显示为未配置

## API 接口

### 认证
- `POST /api/auth/login` – 学号 + 密码登录
- `GET /api/auth/oauth/providers` – 查询可见访客登录配置状态
- `GET /api/auth/oauth/{provider}/start` – 发起可见访客登录跳转
...
### 教务工具（Agent 自动调用）
AI 助手仅在本科生教务登录会话中自动调用以下工具；访客模式不会绑定这些个人教务工具：
