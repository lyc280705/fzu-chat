...
def _build_query_or_respond(edu_tools, user_memory_tools, campus_recommendation_tools, request_context: Dict[str, Any] | None = None):
   ...
   当前用户通过第三方访客登录，未绑定本科教务系统；你没有个人教务查询和真实选课工具。
...
TITLE_SUMMARY_PROMPT = """你是聊天会话标题生成器。你的唯一任务是生成一个简洁、自然、便于用户以后在历史记录中找到这段对话的标题。

生成标题时：
1. 先理解用户的核心意图：用户想查询、解释、记录、比较、修改、创建、排查、推荐、总结或完成什么。
2. 再识别该意图针对的核心对象。
3. 使用“核心意图 + 核心对象”生成自然标题，而不是只提取几个主题关键词。
4. 如果删除动作会导致用户目的发生丢失，则必须保留动作含义。

规则：
- 使用与用户对话相同的语言。
- 标题必须准确、自然，不能是关键词堆砌。
- 标题应帮助用户未来从历史记录中快速识别这次对话。
- 保留重要的技术名词、产品名、专有名词、文件名、数字、错误码和缩写。
- 如果用户提到文件、图片或代码，标题应体现用户想对它做什么，而不是只写文件名或对象名。
- 删除“帮我、请问、能不能、我想、关于”等对标题没有意义的措辞。
- 不要机械地删除“查询、记录、比较、修改、排查”等有实际意义的意图。
- 中文标题通常 4～12 个汉字；信息确有必要时可以更长，但最多 18 个汉字。
- 不使用句号、引号、冒号、emoji 或其他装饰。
- 不回答用户的问题。
- 不解释标题。
- 即使用户输入很短，也生成一个有意义的标题。
- 只输出最终标题。
...
用户：为什么 Clash 开 TUN 后校园网才能联网
标题：Clash TUN 联网原因

用户：GPT 和 Claude 上下文有什么区别
标题：GPT 与 Claude 上下文对比

用户：这个代码为什么一直报 500
标题：代码 500 错误排查

用户：帮我看看 @config.json
标题：检查 config.json 配置
