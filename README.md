...
Current tagged release: [v7.8](CHANGELOG.md)

Release notes: [CHANGELOG.md](CHANGELOG.md)

## Overview

FZU-Chat provides a ChatGPT-style conversation experience for Fuzhou University students. Each student logs in with their student ID to get isolated conversations and access to educational system tools (grade queries, course schedules, etc.); external visitors can use WeChat or QQ guest login for public Q&A and campus-life guidance.

## Features

- **Student authentication**: Per-student login with conversation isolation
- **WeChat/QQ guest mode**: Third-party OAuth creates a visitor session with public Q&A, knowledge retrieval, web search, and campus-life recommendations, without binding personal grade, schedule, selection, or academic-affairs tools
...
│   ├── oauth.py           # WeChat/QQ visitor OAuth helpers
...
- `FZU_CHAT_WECHAT_CLIENT_ID` / `FZU_CHAT_WECHAT_CLIENT_SECRET`, `FZU_CHAT_QQ_CLIENT_ID` / `FZU_CHAT_QQ_CLIENT_SECRET` – Optional WeChat and QQ visitor OAuth credentials; the matching `*_REDIRECT_URI` variables can override the default `/api/auth/oauth/{provider}/callback` callback URL
...
export FZU_CHAT_QQ_CLIENT_ID=... # optional, for visitor QQ login
...
Production deployment can use `docker-compose.prod.yml` with an internal Redis container. Set a URL-safe `REDIS_PASSWORD` such as `openssl rand -hex 32`, then run `FZU_CHAT_VERSION=v7.8 ./scripts/deploy-ghcr.sh`; if GHCR image pull fails, the script falls back to a local production image build.
...
- `FZU_CHAT_WECHAT_CLIENT_ID` / `FZU_CHAT_WECHAT_CLIENT_SECRET`, `FZU_CHAT_QQ_CLIENT_ID` / `FZU_CHAT_QQ_CLIENT_SECRET` – enable the matching visitor login entry; unconfigured providers are shown as unavailable in the login UI

## API Endpoints

### Authentication
- `POST /api/auth/login` – Login with student ID + password
- `GET /api/auth/oauth/providers` – Report WeChat/QQ visitor-login availability
- `GET /api/auth/oauth/{provider}/start` – Start a WeChat or QQ visitor OAuth redirect
- `GET /api/auth/oauth/{provider}/callback` – OAuth callback that creates a visitor session without educational tools
...
The LLM agent can automatically call these tools only in undergraduate educational-login sessions; WeChat/QQ guest mode never binds these personal academic-affairs tools:
