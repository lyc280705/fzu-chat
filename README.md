...
Current tagged release: [v7.12](CHANGELOG.md)
...
Production deployment can use `docker-compose.prod.yml` with an internal Redis container. Set a URL-safe `REDIS_PASSWORD` such as `openssl rand -hex 32`, then run `FZU_CHAT_VERSION=v7.12 ./scripts/deploy-ghcr.sh`; if GHCR image pull fails, the script falls back to a local production image build.
